/**
 * @file Prisma client factory for Phase 2 document review persistence.
 * @author PopoY
 * @created 2026-06-11
 */
import type { PrismaDocumentReviewsClient } from "./reviews.service";

type PrismaClientConstructor = new () => PrismaDocumentReviewsClient & {
 ...
 */
export async function createDocumentReviewsPrismaClient() {
  ...
  try {
    ...
    if (
      !hasPrismaModelFields(prismaModule, "DocumentReview", [
        "documentId",
        "submittedVersionId"
      ])
    ) {
    ...
    // Fall back to in-memory reviews when the local database is unavailable.
    return null;
  }
}
